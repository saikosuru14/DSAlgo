...
public class ArrayProblemsTest {
    public static void main(String[] args) {
        ...
        System.out.println("Is sortedArr sorted? " + arraySort.isSorted(sortedArr));  // true
        System.out.println("Is unsortedArr sorted? " + arraySort.isSorted(unsortedArr));  // false

        // ✅ Test 2: Check for duplicate digits
        ArrayDuplicate arrayDuplicate = new ArrayDuplicate();
        int[] arrWithDuplicates = {1, 2, 3, 4, 2};
        int[] arrWithoutDuplicates = {1, 2, 3, 4, 5};
        System.out.println("Has duplicates? " + arrayDuplicate.isDuplicateDigits(arrWithDuplicates));  // true
        System.out.println("Has duplicates? " + arrayDuplicate.isDuplicateDigits(arrWithoutDuplicates));  // false

        // ✅ Test 3: Two sum (Brute force)
        TwoSum twoSum = new TwoSum();
        int[] numsForTwoSum = {2, 7, 11, 15};
        int target = 9;
        int[] result = twoSum.twoSumBruteForce(numsForTwoSum, target);
        ...
        moveZeroesToEnd.moveZeros(numsWithZeros);  // Output printed inside method
